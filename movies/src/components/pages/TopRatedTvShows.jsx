    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                setTopTvShow(jsonData.results); // Assuming the movie results are in the 'results' property
    ...
    return (
        <div>
            ...
            >
                ...
                {topTvShow.map((tvtop) => (
                    <SwiperSlide key={tvtop.id}>

                        <div className="conteiner-TvShow">
                            <div className="topcard-TvShow">
                                <NavLink to={`tvshows/${tvtop.id}`}>
                                    <img src={`https://www.themoviedb.org/t/p/w220_and_h330_face/${tvtop.poster_path}`} alt="" className='poster-TvShow' />
                                </NavLink>
                            </div>
                            <div className="botomCard-TvShow">
                                <h3 className="cardTitle-TvShow">{tvtop.name}</h3>
                                <p className="cardRating-TvShow">Rating: {tvtop.vote_average}</p>
